...
def is_valid_word(word, hand, word_list):
    ...
    if len(word) > HAND_SIZE:
        print("This word is longer than the "+ HAND_SIZE + "-letter hand size allowed")
        isHandSize = False
    else: isHandSize = True
    
    ## Test if hand has all the letters needed
    handLeft = hand
    for letter in word:
        if handLeft.get(letter, 0) != 0:
            handLeft[letter] -= 1
            isInHand = True
        else:
            print("Letter", letter, "is not available for play")
            isInHand = False
            break

    ## Test if word is in the list
    if word in word_list:
        isInWordList = True
    else:
        print("This word isnot listed our in dictionary")
        isInWordList = False

    ## Test if both conditions are met before returning the validity
    if isHandSize == True and isInHand == True and isInWordList == True:
        isValid = True
    else: isValid = False
    ...
    * The final score is displayed.
    ...
    while len(hand) > 0:
        ...
        if word != '.': 
            ...
            scoreThisHand += get_word_score(word, HAND_SIZE)
            update_hand(hand, word)
            display_hand(hand)

        else: break

    print("Total score in this hand:", scoreThisHand)
